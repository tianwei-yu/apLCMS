...
#' @param min_cont_run filter parameter. 
...
#' @return unique_grp. 
compute_uniq_grp <- function(profile, min_cont_run, min_pres = 0.6) {
  grps <- profile  
  ttt <- table(grps)
  ttt <- ttt[ttt >= max(min_cont_run * min_pres, 2)]
...
#' @export
#' @examples
#' cont_index(newprof, min_pres = min_pres, min_run = min_run)
cont_index <- function(newprof,
                       min_pres = 0.6,
                       min_run = 5) {
  ...
  min_cont_run <- min_run * length(times) / (max(times) - min(times))
  min_run <- round(min_cont_run)

  # computes unique groups 
  uniq_grp <- compute_uniq_grp(newprof$grps, min_cont_run)
  ...
  for (m in 2:length(breaks))
  {
    ...
    if (sum(to_keep) > 0) {
      this_new <- dplyr::slice(this_prof, which(to_keep == 1))
      r.new <- nrow(this_new) 
      new_rec[rec_pointer:(rec_pointer + r.new - 1), ] <- this_new
      rec_pointer <- rec_pointer + r.new
    }
  }
